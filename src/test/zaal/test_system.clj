...
(defn create-auth0-test-user
  ...
  (let [auth0 (-> state/system :zaal.components.auth0/auth)]
...
(defn test-endpoint
  ...
  ([method uri opts]
   (let [app (-> state/system :zaal.server/app)
         request (app (-> (mock/request method uri)
                          (cond->
                           (:auth opts) (mock/header :authorization (str "Bearer " (or @token (get-test-token "testing@zaal.app"))))
                           (:body opts) (mock/json-body (:body opts)))))]
     (update request :body (partial m/decode "application/json")))))

(comment
  (get-test-token "testing@zaal.app")
