(ns clojure-experiment.test-system
  (:require [clojure.test :refer :all]
            [integrant.repl.state :as state]
            [ring.mock.request :as mock]
            [muuntaja.core :as m]
            [clojure-experiment.auth0 :as auth0]
...
(defn create-auth0-test-user
  ...
  (let [auth0 (-> state/system :clojure-experiment.components.auth0/auth)]
...
(defn test-endpoint
  ...
  ([method uri opts]
   (let [app (-> state/system :clojure-experiment.server/app)
         request (app (-> (mock/request method uri)
                          (cond->
                           (:auth opts) (mock/header :authorization (str "Bearer " (or @token (get-test-token "testing@clojure-experiment.app"))))
                           (:body opts) (mock/json-body (:body opts)))))]
     (update request :body (partial m/decode "application/json")))))

(comment
  (get-test-token "testing@clojure-experiment.app")
