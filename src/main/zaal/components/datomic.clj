(ns zaal.components.datomic
  (:require [datomic.client.api :as d]
            [zaal.validation :as validation]
            [clojure.edn :as edn]
...
(defn load-dataset
  ...
  (let [db (d/db conn)
    ...
    (when-not (ident-has-attr? db :account/account-id :db/ident)
      (tx (-> (io/resource "zaal/schema.edn") slurp edn/read-string))
      (tx (-> (io/resource "zaal/seed.edn") slurp edn/read-string)))
